...
def getSesVars(path,dpath,fpath=None,nav=None):
    t,x,y,z = path.T
    t1,dx,dy,dz = dpath.T
    if fpath is not None: t2,fx,fy,fz = fpath.T
    t3,n = nav.T
    assert t[-1] == t1[-1], 'problems with times of path and dpath' 
    if fpath is not None: assert t[-1] == t2[-1], 'problems with times of path and fpath'
    assert t[-1] == t3[-1], 'problems with times of path and nav'
    if fpath is not None: 
        return t,x,y,z,dx,dy,dz,fx,fy,fz,n
    else:
        return t,x,y,z,dx,dy,dz,n
...
def writeJson(pars_path, pars):
    #print('folderParh',folderParh)
    #file = folderPath+'-pars.json'
    with open(pars_path, 'w', encoding='utf-8') as f:
